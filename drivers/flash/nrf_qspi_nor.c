...
#define NRF53_ERRATA_122_PRESENT_QSPI		0
...
struct qspi_nor_data {
	...
	struct k_sem sync;
#if NRF53_ERRATA_122_PRESENT_QSPI
...
static int anomaly_122_init(const struct device *dev)
{
	...
	// if (!nrf53_errata_wasan()) {
	// 	printk("Fail nrf53_errata_wasan\r\n");
	// 	return 0;
	// }
	
	printk("PASS nrf53_errata_wasan\r\n");
...
#endif /* NRF53_ERRATA_122_PRESENT_QSPI */
...
static int enter_dpd(const struct device *const dev)
{
	if (IS_ENABLED(DT_INST_PROP(0, has_dpd))) {
		...
		printk("T_T IS_ENABLED(DT_INST_PROP\n");
	}
			      ...
			      enum pm_device_action action)
{
	...
	case PM_DEVICE_ACTION_SUSPEND:
		printk("qspi PM_DEVICE_ACTION_SUSPEND\n");
